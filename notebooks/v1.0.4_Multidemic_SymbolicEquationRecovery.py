...
# Our oracle on the OTHER two model strings, under the same time limit, so the
# three end-of-run paths can be compared on our ruler as well as SRBench's.
def _nb_oracle(expr_str):
    if expr_str is None:
        return None
    if _main_has_sigalrm:
        _signal_main.alarm(60)
    try:
        r = hgh.equation_recovery_report(
            discovered_expr=sp.sympify(expr_str, locals={v: sp.Symbol(v, real=True)
                                                         for v in finalTerminals}),
            truth_expr=truth_expr, variables=problem.variables, rel_tol_numeric=1e-6,
            var_ranges=_union_ranges(problem.train_ranges, problem.extrap_ranges))
        return bool(r["exact"])
    except _MainRecTimeout:
        return False
...
_pt = time.perf_counter()
experiment["oracle_no_sympy"] = _nb_oracle(model_no_sympy)
experiment["oracle_lint_then_sympy"] = _nb_oracle(model_lint_then_sympy)
_perf_add("oracle: the two extra model strings", _pt)

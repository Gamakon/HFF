...
def _job_inner(name, seed, tn, out):
    ...
    out.update({k: alg.LAST_FIT.get(k) for k in ("generations", "population", "individuals")})
...
def main():
    ...
    print(f"{'dataset':<22}{'seed':>6}{'noise':>7}{'r2_test':>10}{'size':>6}{'fit s':>7}{'gens':>6}{'pop':>6}{'indiv':>9}  sol  status / model", flush=True)
    t0 = time.time()
    ...
    with mp.Pool(args.workers, initializer=_init, initargs=(args.max_time, args.results, deadline)) as pool:
        for r in pool.imap_unordered(_job, jobs):
            ...
            print(f"{r['dataset']:<22}{r['seed']:>6}{r['noise']:>7}"
                  ...
                  f"{r.get('fit_wall', 0):>7.1f}{str(r.get('generations', '-')):>6}{str(r.get('population', '-')):>6}"

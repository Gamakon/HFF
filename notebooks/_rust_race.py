...
def main():
    ...
    print(f"RUST ENGINE RACE: {len(names)} datasets | development seed {seed} | {args.seconds:.0f} s each | population {args.population} | one fit at a time", flush=True)
    ...
    for name in names:
        ...
        t = time.time()
        run = subprocess.run([ENGINE, train_path, str(seed), str(args.seconds), str(args.max_rows), str(args.population), "all"],

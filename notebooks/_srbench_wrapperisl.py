...
Logs: tail -f /tmp/srbench_wrapperisl.log   |   rows: /tmp/srbench_wrapperisl.jsonl
"""
...
LOG = "/tmp/srbench_wrapperisl.log"
JSONL = "/tmp/srbench_wrapperisl.jsonl"
SEED = 11
N_PROBLEMS = 9
BUDGET = 900.0      # safety ceiling; N_GEN=200 is the intended stop
N_GEN = 200         # matched to _srbench_200g for the A/B
POP_INTAKE = 100
POP_CHAMP = 50
PARS_COL_MAX = 200.0
...
def run_problem(name: str) -> dict:
    ...
    os.environ["HFF_HOF_DUMP"] = f"/tmp/hff_hof_wrapperisl_{safe_name}.pkl"
    ...
    cfg = HFFSRConfig(
        ...
        # Generation-starvation fix (90m run reached only ~35 gens/600s at
        # pop 400/150 with fuller ops on every offspring): small pop, fuller
        # throttled, so the same wall clock buys hundreds of generations.
        pb_physics=0.10, pb_denoise=0.05, snap_winners_top_k=10,
        # A/B vs _srbench_smallpop: same pop per class; each individual
        # evaluates 1 wrapper instead of 3, so per-gen eval cost of the
        # 3x population is roughly a wash.
        wrapper_islands=True,
    )
    eng = HFFSREngine(cfg)
...
def main():
    ...
    log(f"=== SRBench wrapper-per-island sweep START === {N_PROBLEMS} problems, seed={SEED}, "
